...
class Monitor
{
    ...
    public:
		...
		{
			//gen 50 processes
			//memory.resize(10000,0); // initilize all to 0
			memory50.resize(5000,0); //50% of memeory
		...
		void get_chunks() // call after every malloc and free
		{
			...
			while(i < 4999) //check all memeory
			{
				if(memory50.at(i) != 0)
				{
					i += memory50.at(i); //get the index for the block of mem right after this process
				}
				else
				{
					index = i; // first index of contigious block
					counter =0;
					while(memory50.at(i) == 0)
					{
						if(i >= 4999)
							break;
						i++;
		...
		void my_malloc(Process *p) //assigning data
		{
			...
			memory50.at(index) = size; //allocate size for the process in mem
